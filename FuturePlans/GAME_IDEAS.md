...
**Status:** **SHIPPED (v3) · LOCKED** — `assets/games/chicken-chaos/index.html` · **Source:**
`Chicken Chaos.html` is the master reference copy only (never a build input)

- **Hook:** grab the chicken, dash away, don't get tackled — a same-screen scramble where everyone
  fights over one bird.
- **The shipped rules (v2):** **SCORE = CHICKENS CAUGHT.** There is no target score and nobody can
  end a match early; the match always runs `MATCH_TIME` (60 s) and the player with the most catches
  takes it (a tie is called as a tie).
- **What the shipped copy carries:** the `STUDIO_GAME_MANIFEST` (v3), the Studio SDK block with
  banner-free rewarded + **interstitial on the result-card buttons** helpers, the full `window.Game` lifecycle
  (`pause` / `resume` / `destroy` / `setMuted` / `setSettings` / `onOrientationChange`), shell
  settings sync (sound / haptics / volume), a gesture-gated audio unlock, and the **landscape turn
  owned by the shell** (`SCREEN_ORIENTATION_SENSOR_LANDSCAPE`) — the game's own WebView
  `requestFullscreen()` hack is gone.
- **v2 pass (2026-09-23):** square, larger catch boxes (number only, no `/10`) flanking the timer
  and the pause button; in-game texts (`GO!` / hint pill / `TIME UP!`) sized from the viewport with
  auto-fit instead of fixed 100–140 px; chicken slowed (208 → 168, panic 1.10, turn 4.6); per-second
  warning ticks over the last 10 s (harder inside the last 5); DASH icon + caption centred on the
  button; result card rebuilt in the game's cream/wood language (winner band, one catch tile per
  player, PLAY AGAIN / MENU); back / gear / pause chrome themed and drawn without a "cut" gloss;
  first tap is audible again.
- **v3 pass (2026-09-23, after two rounds of device feedback):** every in-game text is a caption
  rather than a billboard — `GO!`, the countdown digits and `TIME UP!` are `ch*0.095–ch*0.115`
  (30–56 px here, ~1.3× the timer digits) and the hint pill is `ch*0.036` (12–16 px), hugging **its
  own text** (`measureTxt`) and moved into the **bottom band** so it stops crowding the HUD strip.
  Back / gear are the game's own **unselected toggle button** — the `rgba(60,50,35,.72)` face with the
  `#241c10` outline of LOCAL / VS BOTS when they are OFF — with white icons, and the in-game pause
  keeps its **original dark translucent disc**; the `DASH` caption is gone and the two mirrored
  chevrons sit on the exact button centre. Catch boxes are square, number-only and a little **smaller
  than the round pause button**. The result card is narrower (658 px here, was 90 % of the screen) and
  its board is a cream-to-wood vertical gradient; the halves do the work: **LEFT** a grass paddock
  sized to the bird with the **START-PAGE chicken** standing on it — plain white, no colour wash,
  drawn straight onto the canvas so the comb and beak are no longer cropped by the old sprite box —
  and **RIGHT** the win / tie headline in the **TIME UP! styling** (yellow `#ffd23e` with the
  dark-brown outline) plus a 6 px accent rule, `CHICKENS CAUGHT`, a one-row-per-player catch table
  (colour chip, colour name, count, the winner's row tinted **and** tagged `WINNER`) and
  PLAY AGAIN / MENU. No colour band, no stars. The interstitial no longer fires on match end (it
  landed on the result in the same frame): it fires when the player **chooses** — PLAY AGAIN or MENU —
  and the tap then continues into the new match / the start page. Verified headlessly:
  `node _tmp/cc_headless.js` → ALL CHECKS PASSED (56 assertions, including the paddock and white
  chicken, the box-vs-pause size, the hint's bottom band, the headline style match and the ad-on-tap
  timing), and `_tmp/cc_blueprint.py` renders the real draw calls with their real colours to
  `_tmp/cc_result_card.png` for a layout eyeball.
- **Tier 1, landscape, 2–4 players + bots.** Tile colour `#6BCB4A` (`tile_chicken_chaos.jpg`).
...
> **One prompt file per game, ready to hand:** `prompts/<manifest-id> prompt.md` - one for every
> game on the status board, generated by `_gen_game_prompts.py` from §1, §6 and §12. A session
> starts by pasting a single file instead of reading four sections. `GAME_IDEAS.md` stays the
> single source of truth and the files are overwritten on every generator run, so edit an entry
> here, re-run the script, and hand the result over.
